...
//Aqui inicia los efectos de diego
// Variables adicionales para el efecto de niebla
let ctx1: CanvasRenderingContext2D = pantalla2;
let w1: number;
let h1: number;
let particlesArray1: FogParticle[] = [];

function initializeFogEffect() {
    // Inicialización
    let imagenSal: ImageType = new ImageType(pantalla1, imgLocal.getImage());
    w1 = imagenSal.getWidth();
    h1 = imagenSal.getHeight();

    // Creación de partículas para simular la niebla
    for (let i = 0; i < numberOfParticles; i++) {
        particlesArray1.push(new FogParticle(w1, h1, ctx1));
    }
}

let fogImage: HTMLImageElement = new Image(); // Crear una nueva imagen para la niebla
fogImage.src = 'nube.png'; // Reemplaza con la ruta correcta de tu imagen de niebla

let fogOffsetX: number = 0; // Inicializa el desplazamiento en el eje X
...
function startFogEffect(evt: any): void {
    initializeFogEffect();
    animateFogEffect();
}
...
class FogParticle {
    ...
    speed: number = 1; // Aumenté la velocidad para que sea más evidente
...
function startBouncingBalls(evt: any): void {
...
// Código para efecto de partículas de texto
...
function textEffects(evt: any): void {
    let args: string | null = prompt("Ingresa texto, tamaño de texto y coord x y y, separados por coma:");

    if (args) {
        let factors: string[] = args.split(','); //.map(elem => parseInt(elem));
        pantalla1.font = 'bold ' + factors[1] + 'px Verdana';
        pantalla1.fillText(factors[0], parseInt(factors[2]), parseInt(factors[3]));
